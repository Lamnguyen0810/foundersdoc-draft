...
**Feedback** beside every draft (admins only): a lawyer says what is wrong, with the passage they selected. Or type a message beginning `feedback:` in the Slack drafts channel — Zapier (Slack "New Message Posted to Channel", bot messages off → Webhooks POST to `/api/feedback/slack`, header `x-feedback-secret` from `select public.slack_feedback_secret()`, body `text`/`user_name`/`permalink`) forwards it. Both land in **Admin → AI files → Feedback & lessons** — and the drafter learns from them by itself (`lib/learn.ts`, `supabase/046`): the comment is read against the draft it was about (name it with the `#ref` from the Slack download message: `fb #3f9a1c: …`), the playbook and the rules already learnt, and rewritten as one rule, live from the next draft, announced in Slack, editable and switchable on the dashboard. Feedback that is not a correction waits for a person; "Turn into rule" and "Add a rule" still work by hand. `fb undo` in Slack switches off the most recent live rule (`supabase/047`). Live lessons go into every later draft of that scope after the playbook, with the same authority (`lessonsBlock()`). Storage is `supabase/045_feedback_and_lessons.sql` and `046_the_drafter_learns.sql`.

### The playbook's conventions are the app's

With a playbook live, the built-in prompt's own conventions for gaps and notes give way to the playbook's: `[●]` for a missing fact and `[FD Note: …]` in the text (shown highlighted on screen and in Word, and listed in the FD Notes card beside the document); no DRAFTER'S NOTES block is asked for. The page and the Word file are set from the playbook's own style parameters — a `house_style` block (`font`, `body_size_pt`, `title_size_pt`, `line_spacing`, `space_after_pt`, `heading_space_before_pt`, `heading_space_after_pt`, `alignment_body`) or the same in prose (`documentLook()` in `lib/playbook.ts`). Numbering is read three sub-levels deep: (a), (i), (A).
