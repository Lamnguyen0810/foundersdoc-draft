/**
 * Turns the uploaded user settings design into a stylesheet this app can serve.
 *
 *   node scripts/build-settings-css.mjs <design.html> > src/app/settings/settings.css
 *
 ...
 *
 * 2. NAMESPACES everything under `.fdset`. globals.css is 1,600 lines and already
...
if (!src) { console.error("usage: build-settings-css.mjs <design.html>"); process.exit(1); }
...
const DROP = [".header", ".nav", ".brand", ".has-sub", ".theme-toggle", ".account",
              ".nav-cta", ".nav-links", ".nav-left", ".nav-right", ".nav-sub"];
...
console.log(" * Source: the uploaded user settings design, via scripts/build-settings-css.mjs.");
