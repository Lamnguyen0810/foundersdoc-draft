/**
 * Turns the uploaded admin dashboard design into a stylesheet this app can serve.
 *
 *   node scripts/build-admin-css.mjs <design.html> > src/app/admin/dashboard.css
 *
 ...
 *
 * 2. NAMESPACES everything under `.fdadmin`. globals.css is 1,600 lines and already
...
if (!src) { console.error("usage: build-admin-css.mjs <design.html>"); process.exit(1); }
...
const DROP = [".topbar", ".nav", ".mainnav", ".brand", ".nav-actions", ".user-chip",
              ".user-copy", ".avatar", ".dropdown", ".nav-link", ".nav-item",
              ".nav-chevron", ".fdref-", ".has-sub", ".nav-sub"];
...
function scopeSelector(sel) {
  return sel.split(",").map((one) => {
    ...
    if (!s) return null;
    if (s === "*") return ".fdadmin *";
    if (s === ":root" || s === "html" || s === "body") return ".fdadmin";
    /* `html[data-theme="dark"] { --tokens }` is the design's dark palette. The
       attribute lives on the real <html>, so the scope goes AFTER it. */
    if (/^html\[/.test(s)) return s.replace(/^html(\[[^\]]+\])\s*/, "html$1 .fdadmin ");
    if (s.startsWith(".fdadmin")) return s;
    return ".fdadmin " + s;
  }).filter(Boolean).join(",");
}
...
console.log(" * Source: the uploaded admin dashboard design, via scripts/build-admin-css.mjs.");
