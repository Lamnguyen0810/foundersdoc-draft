...
type Tab = "documents" | "people" | "billing";

const TABS: { id: Tab; label: string }[] = [
  { id: "documents", label: "Documents" },
  { id: "people", label: "User statistics" },
  { id: "billing", label: "Credits & plans" },
];
...
export default async function AdminPage({
  ...
  const tab: Tab = TABS.some((t) => t.id === sp.tab) ? (sp.tab as Tab) : "documents";
  ...
  const [
    ...
    creditLog,
      ...
      tab === "documents"
        ...
        : null,
      tab === "people" ? supabase.rpc("admin_user_stats", { p_days: days }) : null,
      tab === "people" ? supabase.rpc("admin_funnel", { p_days: days }) : null,
      tab === "people"
      ...
      tab === "people"
        ? supabase
            ...
            .limit(200)
        : null,
      tab === "billing" ? supabase.rpc("admin_plan_stats", { p_days: days }) : null,
      tab === "billing"
        ? supabase.rpc("admin_accounts", { p_limit: 200, p_search: q || null })
        : null,
      tab === "billing" ? supabase.rpc("admin_recent_credit_actions", { p_limit: 15 }) : null,
    ]);
    ...
    <main className="fda">
      <div className="fda-head">
        <div>
          <h1>Admin</h1>
          <p>
            Everything here is counted from the database. Where a panel is empty it says what would
            have to happen to fill it — no figure on this page is estimated or seeded.
          </p>
        </div>
        <div className="fda-tools">
          <div className="range" role="group" aria-label="Date range">
            {RANGES.map((r) => (
              <Link
                key={r.days}
                href={tabHref(tab, r.days, q)}
      ...
      <nav className="fda-tabs" aria-label="Admin sections">
        {TABS.map((t) => (
          <Link
            key={t.id}
